...
static int proto_gen_action(const iotex_st_act_core *core,
    ...
    {
        if (i > 0) printf(":");
        printf("%02X", action_bytes[i]);
    }
    printf("\n");printf("\n");

    printf("private_key_bytes--> ");
    for (int i = 0; i < sizeof(private_key_bytes); i++)
    {
        if (i > 0) printf(":");
        printf("%02X", private_key_bytes[i]);
    }
    printf("\n");printf("\n");

    printf("action_signature--> ");
    for (int i = 0; i < sizeof(action_signature); i++)
    {
        if (i > 0) printf(":");
        printf("%02X", action_signature[i]);
    }
    printf("\n");printf("\n");
